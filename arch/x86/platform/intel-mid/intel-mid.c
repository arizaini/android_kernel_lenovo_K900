					...
					struct devs_id *dev)
{
	...
	pr_info("SPI bus = %d, name = %16.16s, "
		...
		"platform_data=0x%x, controller_data=0x%x\n",
		spi_info.bus_num,
		spi_info.modalias,
		spi_info.irq,
		spi_info.max_speed_hz,
		spi_info.chip_select,
		spi_info.platform_data,
		spi_info.controller_data);
					...
					struct devs_id *dev)
{
	...
	void *pdata = NULL;
//	static int done, wm2000_done;
	static int wm2000_done;

	/* Bodge to register WM5102, should be done by the SFI not
	 * open coded here.
	 */
#if 0
	if (!done) {
		memset(&i2c_info, 0, sizeof(i2c_info));
		strncpy(i2c_info.type, "wm5102", sizeof(i2c_info.type));
		i2c_info.addr = 0x1a;
