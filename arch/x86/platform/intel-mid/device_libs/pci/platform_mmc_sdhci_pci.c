...
extern void sdhci_wifi_status_register(void *dev_id, void (*virtual_cd)
...
void bcmdhd_register_embedded_control(void *dev_id, void (*virtual_cd)
					(void *dev_id, int card_present))
{
	sdhci_wifi_status_register(dev_id, virtual_cd);
	return;
}
...
static int __init sdhci_pci_platform_data_init(void)
{
    ...
    mfd_clv_sdio_data.mmc_caps = bcmdhd_get_sdhci_mmc_caps();
