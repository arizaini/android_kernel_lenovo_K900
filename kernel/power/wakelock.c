...
static struct work_struct suspend_sync_work;
...
static bool check_sys_sync(void)
{
	while (!wait_for_completion_timeout(&suspend_sync_complete,
		...
		if (has_wake_lock(WAKE_LOCK_SUSPEND))
			return false;
		/* If sys_sync is doing, but no wakelock alive,
...
static int __init wakelocks_init(void)
{
	...
	INIT_WORK(&suspend_sync_work, do_suspend_sync);
