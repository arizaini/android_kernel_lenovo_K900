...
/*
 * The suspend/resume is pretty much similar to the legacy
...
{
    if(sst_drv_ctx->sst_state == SST_SUSPENDED){
        pr_err("System already in Suspend state\n");
        return 0;
    }

    if(sst_drv_ctx->dma.dmac){
        if(!pm_runtime_suspended(&sst_drv_ctx->dma.dmac->dev)){
            pm_schedule_suspend(&sst_drv_ctx->dma.dmac->dev, 50);
            pr_err("dma busy, schedule suspend later\n");
            return -EBUSY;
        }
    }

    return intel_sst_suspend(dev);
}
