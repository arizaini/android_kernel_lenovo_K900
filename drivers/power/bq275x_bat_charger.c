				...
				struct power_supply_charger_cap *cap)
{
	...
	printk("%s:[chrg] evt:%d type:%d cur:%d bat_status:%d\n",__func__,
				cap->chrg_evt, cap->chrg_type, cap->mA,bat_status);
	power_supply_changed(&di->usb);
...
static void bq24192_interrupt_worker(struct work_struct *work)
{
	...
	else if (di->cap.chrg_type == 3){

		di->usb_online = 1;
	}else if(di->cap.chrg_type == 0){
		di->mains_online = 0;
		di->usb_online = 0;
	}
