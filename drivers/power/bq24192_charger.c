...
/* IRQ handler for pmic charger Interrupts configured to USBDET irq */
static irqreturn_t pmic_charger_intr_handler(int irq, void *devid)
{
	struct bq24192_chip *chip = (struct bq24192_chip *)devid;
	/* This hanlder is used for USB_DET handling */
	dev_info(&chip->client->dev,
		"IRQ Handled for  pmic charger interrupt: %d\n", irq);
			...
			const struct i2c_device_id *id)
{
	...
	/* Register a handler for Charger Interrupt gpio pin */
	ret = request_irq(chip->pdata->pmic_irq,
			pmic_charger_intr_handler,
			IRQF_TRIGGER_FALLING,
			"pmic_usbdet_interrupt",
			chip);
	if (ret) {
		dev_warn(&bq24192_client->dev,
		"failed to register pmic interrupt %d\n", ret);
		chip->pdata->pmic_irq = 0;
	} else {
		dev_info(&bq24192_client->dev,
			"registered pmic interrupt for irq %d\n",
				 chip->pdata->pmic_irq);
	}
	...
	if (!chip->pdata->slave_mode) {
		...
		if (ret) {
			...
			i2c_set_clientdata(client, NULL);
			if (chip->pdata->pmic_irq != 0)
				free_irq(chip->pdata->pmic_irq, chip);
			kfree(chip);
			return ret;
		}
	}
