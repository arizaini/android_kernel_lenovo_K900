...
static int mt9e013_change_fps_regs(const struct mt9e013_reg *reglist,
				   u8 new_fps, u16 *line_pck_out)
{
	const struct mt9e013_reg *next = reglist;
	int line_pck = 1, frame_length = 1;
	int found_line_pck = 0, found_frame_length = 0;
	u16 reg_val;

	for (; next->type != MT9E013_TOK_TERM; next++) {
		reg_val = next->reg.sreg;
		if (reg_val == MT9E013_LINE_LENGTH_PCK) {
			line_pck = next->val;
			found_line_pck = 1;
		} else if (reg_val == MT9E013_FRAME_LENGTH_LINES) {
			frame_length = next->val;
			found_frame_length = 1;
		}
	}

	if ((!found_frame_length) || (!found_line_pck))
		return -1;

	line_pck = (MT9E013_MCLK * 1000000) / (new_fps * frame_length);

	*line_pck_out = line_pck;
	return 0;
}
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
    ...
    if (dev->fps <= mt9e013_res[dev->fmt_idx].fps) {
		ret = mt9e013_change_fps_regs(mt9e013_def_reg,
					      dev->fps, &new_line_pck);
		if (!ret)
			mt9e013_write_reg(client, MT9E013_16BIT,
					   MT9E013_LINE_LENGTH_PCK,
					   new_line_pck);
	}
...
/*	fival->width = mt9e013_res[index].width;
	...
	fival->discrete.denominator = dev->fps;
...
mt9e013_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *param)
{
	...
	if ((dev->fps >= MIN_FPS) && (dev->fps <= MAX_FPS)) {
...
mt9e013_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *param)
{
	...
	dev->fps = mt9e013_res[0].fps;
	if ((param->parm.capture.timeperframe.denominator > 0)
		&& (param->parm.capture.timeperframe.numerator > 0)) {
		u8 tempfps = (param->parm.capture.timeperframe.denominator)/
			((param->parm.capture.timeperframe.numerator)*10000000);

		if ((tempfps >= MIN_FPS) && (tempfps <= MAX_FPS))
			dev->fps = tempfps;
	}
