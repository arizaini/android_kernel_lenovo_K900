			...
			enum hmm_pool_type pool_type)
{
	...
	case HMM_POOL_TYPE_RESERVED:
		...
		ret = reserved_pool.pops->pool_init(&reserved_pool.priv_data,
							pool_size);
		break;
	case HMM_POOL_TYPE_DYNAMIC:
		dynamic_pool.pops = &dynamic_pops;
		ret = dynamic_pool.pops->pool_init(&dynamic_pool.priv_data,
							pool_size);
		break;
	default:
...
void hmm_pool_unregister(enum hmm_pool_type pool_type)
{
	...
	case HMM_POOL_TYPE_RESERVED:
		if (reserved_pool.pops->pool_exit)
			reserved_pool.pops->pool_exit(&reserved_pool.priv_data);
		break;
	case HMM_POOL_TYPE_DYNAMIC:
		if (dynamic_pool.pops->pool_exit)
			dynamic_pool.pops->pool_exit(&dynamic_pool.priv_data);
		break;
	default:
		v4l2_err(&atomisp_dev, "invalid pool type.\n");
		break;
