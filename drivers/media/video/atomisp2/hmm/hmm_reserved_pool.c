 ...
 */
static unsigned int get_pages_from_reserved_pool(void *priv_data,
...
static void free_pages_to_reserved_pool(void *priv_data,
					struct hmm_page_object *page_obj)
{
	unsigned long flags;
	struct hmm_reserved_pool_info *repool_info;
	...
	if (repool_info->index < repool_info->pgnr
...
static int hmm_reserved_pool_inited(void *priv_data)
{
	struct hmm_reserved_pool_info *repool_info;
	...
	return repool_info->flag == HMM_RESERVED_POOL_INITED;
}
