	...
	for (i = 0; i < free_pgnr; i++) {
		...
		case HMM_PAGE_TYPE_RESERVED:
			if (repool->pops->pool_free_pages)
				repool->pops->pool_free_pages(repool->priv_data,
							&bo->page_obj[i]);
			break;
		case HMM_PAGE_TYPE_DYNAMIC:
			if (dypool->pops->pool_free_pages)
				dypool->pops->pool_free_pages(dypool->priv_data,
							&bo->page_obj[i]);
			break;
		/*
		...
		case HMM_PAGE_TYPE_GENERAL:
			if (dypool->pops->pool_inited
			    && dypool->pops->pool_inited(dypool->priv_data)) {
				if (dypool->pops->pool_free_pages)
				...
				struct hmm_pool *repool)
{
	...
	if (dypool->pops->pool_alloc_pages) {
		alloc_pgnr = dypool->pops->pool_alloc_pages(dypool->priv_data,
							bo->page_obj, pgnr);
		if (alloc_pgnr == pgnr)
			return 0;
	}
	...
	if (repool->pops->pool_alloc_pages) {
		alloc_pgnr = repool->pops->pool_alloc_pages(repool->priv_data,
							&bo->page_obj[i], pgnr);
		if (alloc_pgnr == pgnr)
			return 0;
	}
...
{
	int ret;
	struct hmm_pool *dypool = &dynamic_pool;
	struct hmm_pool *repool = &reserved_pool;
...
void hmm_bo_free_pages(struct hmm_buffer_object *bo)
{
	...
	if (bo->type == HMM_BO_PRIVATE)
		free_private_pages(bo, dypool, repool);
