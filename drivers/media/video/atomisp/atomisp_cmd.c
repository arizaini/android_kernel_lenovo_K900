...
int atomisp_set_fmt(struct video_device *vdev, struct v4l2_format *f)
{
	...
	/* getting frame rate parameters from driver */
	ret = v4l2_subdev_call(isp->inputs[isp->input_curr].camera,
				video, g_parm, &sensor_parm);
	if (ret) {
		v4l2_err(&atomisp_dev, "can not get parameter from driver, fps setting may failed\n");
		sensor_parm.parm.capture.timeperframe.denominator = 0;
		sensor_parm.parm.capture.timeperframe.numerator = 0;
	}
