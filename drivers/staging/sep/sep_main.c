...
static int sep_wait_for_scu(struct sep_device *sep)
{
	...
	//delay 200ms to 2000ms to wait firmware of SEP boot up 
	...
	dev_dbg(&sep->pdev->dev, "iteration %d times, delta=%lld ns\n",
		delay_count, t2-t1);

	if (gpr3_contents == 0) {
		dev_err(&sep->pdev->dev, "scu boot bit not set at resume\n");
		dev_err(&sep->pdev->dev, "iteration %d times, delta=%lld ns\n",
		delay_count, t2-t1);
		BUG_ON(1);
		return -EINVAL;
	}
