...
extern bool power_button_pressed;
...
static irqreturn_t gpio_keys_isr(int irq, void *dev_id)
{
	...
	if (button->code == KEY_VOLUMEUP && trigger_panic ) {
		if (state) { /* key pressed  */
			...
			panic_start = jiffies;
			pr_info("Key: gpio %d, code %d pressed at %ul\n",
				button->gpio, button->code, panic_start);
		} else {     /* key released */
			bool is_timeout = false;
			unsigned long now = jiffies;

			pr_info("Key: gpio %d, code %d released at %ul, (start = %ul)\n",
				button->gpio, button->code, now, panic_start);
			if (!volume_up_pressed) {
				pr_info("Key: gpio %d, code %d press event arrived "
					"before press\n", button->gpio, button->code);
				goto no_panic;
			}
			is_timeout = time_after(now,
					(panic_start + msecs_to_jiffies(10000)));
			if (is_timeout && power_button_pressed) {
				pr_err("Panic triggered [Vol up] and [Power button]:\n");
				dump_stack();
				BUG();
				while (true) /* should not arrive here */;
			}
			volume_up_pressed = false;
			panic_start = 0xffffffff; /* clean the start jiffies */
		}
	}
no_panic:
#endif
