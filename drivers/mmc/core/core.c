...
static int mmc_rescan_try_freq(struct mmc_host *host, unsigned freq)
{
	...
	printk("mmc_hostname[%s]mmc_rescan_try_freq  mmc_attach_sdio  \n",mmc_hostname(host));
	if (!mmc_attach_sdio(host))
		return 0;
	printk("mmc_hostname[%s]mmc_rescan_try_freq  mmc_attach_sd  \n",mmc_hostname(host));
...
void mmc_rescan(struct work_struct *work)
{
	...
	if(!memcmp(mmc_hostname(host), "mmc2", 4)) {
		if (wifi_on == 0) {
			printk("%s:wifi is off \n", __FUNCTION__);
			/*   if there is a card registered, check whether it is still present */
			if ((host->bus_ops != NULL) && !host->bus_dead) {
				printk("%s: wifi is not detected \n", __FUNCTION__);
