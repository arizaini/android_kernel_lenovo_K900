			    ...
			    struct dlp_xfer_ctx *xfer_ctx)
{
	...
	while (xfer_ctx->wait_len > 0) {
		...
		do {
			...
			while ((data_size) && (copied)); //hzz from intel
...
static int dlp_tty_port_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	if (ret)
		pr_err(DRVNAME ": %s (open_channel failed :%d)\n",__func__, ret);
  		...
  		ret1 += gpio_export(178, 1);
  		ret1 += gpio_export(113, 1);               

#if 0
  		ret1 += gpio_request(115, "ifxHSIModem");
  		ret1 += gpio_export(115, 1);                                 
		gpio_free(115);                                 

  		ret1 += gpio_request(51, "SPH_CS_N");
  		ret1 += gpio_export(51, 1);                                    
   		gpio_free(51);    
  		ret1 += gpio_request(169, "SPH_RST_N");                                         
  		ret1 += gpio_export(169, 1);                                                                       
  		gpio_free(169);    
#endif
		pr_warn(DRVNAME "export usb gpio for usb switch%d\n", ret1);
	}
