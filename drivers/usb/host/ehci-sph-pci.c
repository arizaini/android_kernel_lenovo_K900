...
static int sph_gpio_init(struct pci_dev *pdev)
{
    ...
    if (gpio_is_valid(USB_AP_CP_SWITCH)) {
        retval = gpio_request(USB_AP_CP_SWITCH, "USB_AP_CP_SWITCH");
        if (retval < 0) {
            printk(KERN_INFO "Request GPIO %d with error %d\n",
            USB_AP_CP_SWITCH, retval);
            retval = -ENODEV;
            goto err1;
        }
    } else {
    ...
    gpio_export(sph_pdata->gpio_cs_n, 1);
    gpio_export(sph_pdata->gpio_reset_n, 1);
    gpio_export(USB_AP_CP_SWITCH, 1);
...
static void sph_gpio_cleanup(struct pci_dev *pdev)
{
	...
	if (!sph_pdata->has_gpio)
		return;
	if (gpio_is_valid(sph_pdata->gpio_cs_n)) {
        gpio_unexport(sph_pdata->gpio_cs_n);
		gpio_free(sph_pdata->gpio_cs_n);
    }
	if (gpio_is_valid(sph_pdata->gpio_reset_n)) {
        gpio_unexport(sph_pdata->gpio_reset_n);
		gpio_free(sph_pdata->gpio_reset_n);
    }

    if (gpio_is_valid(USB_AP_CP_SWITCH)) {
        gpio_unexport(USB_AP_CP_SWITCH);
