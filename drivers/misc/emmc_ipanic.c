				      ...
				      loff_t to, const u_char *buf)
{
	...
	printk(KERN_INFO "%s: start_block(%d), block_count(%d), write to (%d)\n",
			__func__, emmc->start_block, emmc->block_count, to);
	if (to >= emmc->start_block + emmc->block_count) {
		printk(KERN_EMERG "%s: panic partition is full.\n", __func__);
		return 0;
	} else if (to < emmc->start_block) {
		printk(KERN_EMERG "%s: wrong address(%d) to write.\n", __func__, to);
		return 0;
	}
				       ...
				       unsigned long action, void *data)
{
	...
	case BUS_NOTIFY_BOUND_DRIVER:
		...
		if (emmc->disk_device) {
			...
			printk(KERN_INFO "\tstart_block(%d), block_count(%d)\n",
					emmc->start_block, emmc->block_count);
