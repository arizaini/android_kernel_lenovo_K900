				...
				size_t count, loff_t *ppos)
{
	...
	struct circ_buf *xmit = &up3->port.state->xmit;
	char *buf;
	u32 len = 0;
	ssize_t ret;
...
static ssize_t gps_dbg_rd(struct file *file, char __user *user_buf,
				size_t count, loff_t *ppos)
{
	struct uart_hsu_port *up = file->private_data;
	ssize_t ret = 0;
...
static irqreturn_t dummy_irq(int irq, void *dev)
{
	return IRQ_HANDLED;
}

static ssize_t show_wakeup(struct file *file, char __user *user_buf,
				size_t count, loff_t *ppos)
{
	struct uart_hsu_port *up = file->private_data;
	extern void intel_mid_hsu_show_wakeup(int port);
	int ret;

	pr_info("%s\n", __func__);
	gpio_direction_input(96+30);
	lnw_gpio_set_alt(96+30, LNW_GPIO);
	udelay(100);
	ret = request_irq(gpio_to_irq(96+30), dummy_irq,
			IRQ_TYPE_EDGE_FALLING | IRQ_TYPE_EDGE_RISING,
			"gps_rx_pin", NULL);
	return 0;
}

static ssize_t show_rx(struct file *file, char __user *user_buf,
				size_t count, loff_t *ppos)
{
	struct uart_hsu_port *up = file->private_data;
	extern void intel_mid_hsu_show_rx(int port);

	pr_info("%s\n", __func__);
	free_irq(gpio_to_irq(96+30), NULL);
	lnw_gpio_set_alt(96+30, 1);
	return 0;
}
...
static const struct file_operations gps_dbg_ops = {
	.owner		= THIS_MODULE,
	.open		= hsu_show_regs_open,
	.read		= gps_dbg_rd,
	.llseek		= default_llseek,
...
static const struct file_operations show_wakeup_ops = {
	.owner		= THIS_MODULE,
	.open		= hsu_show_regs_open,
	.read		= show_wakeup,
	.llseek		= default_llseek,
...
static const struct file_operations show_rx_ops = {
	.owner		= THIS_MODULE,
	.open		= hsu_show_regs_open,
	.read		= show_rx,
	.llseek		= default_llseek,
...
static int hsu_debugfs_init(struct hsu_port *hsu)
{
	...
	snprintf(name, sizeof(name), "gps_dbg");
	debugfs_create_file(name, S_IFREG | S_IRUGO,
		hsu->debugfs, (void *)(&hsu->port[3]), &gps_dbg_ops);

	snprintf(name, sizeof(name), "show_wakeup_dbg");
	debugfs_create_file(name, S_IFREG | S_IRUGO,
		hsu->debugfs, (void *)(&hsu->port[3]), &show_wakeup_ops);

	snprintf(name, sizeof(name), "show_rx_dbg");
	debugfs_create_file(name, S_IFREG | S_IRUGO,
		hsu->debugfs, (void *)(&hsu->port[3]), &show_rx_ops);
...
static bool allow_for_suspend(struct uart_hsu_port *up)
{
	...
	if ((!up->running) && (up->index == logic_idx) && serial_hsu_ports[share_idx]->running) {
		up = serial_hsu_ports[share_idx];
	}
