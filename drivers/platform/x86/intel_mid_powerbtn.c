...
#ifdef CONFIG_FASTPOWER
static struct device *pwrbtn_dev;
void *get_pwrbtn_dev(void)
{
	if (pwrbtn_dev)
		return pwrbtn_dev;
	return NULL;
}
EXPORT_SYMBOL(get_pwrbtn_dev);
static enum hrtimer_restart fastpower_timer_function(struct hrtimer *__timer)
{
	struct mfld_pb_priv *priv = (struct mfld_pb_priv *)container_of(__timer, struct mfld_pb_priv, timer);
	set_fast_powerup_status(FASTPOWER_UP_PWRBTN_MODE);
...
EXPORT_SYMBOL(power_button_pressed);
#endif
...
#ifdef CONFIG_FASTPOWER
	pr_debug("(pbstat & MSIC_PB_LEVEL): 0x%x \n", (pbstat & MSIC_PB_LEVEL));
	if(FASTPOWER_OFF_MODE == get_fast_poweroff_status())
	{
		if(0 == (pbstat & MSIC_PB_LEVEL))	// btn press
		{
			hrtimer_start(&priv->timer, ktime_set(2, 0), HRTIMER_MODE_REL);
			pr_info("power key press: \n");
			return IRQ_HANDLED;
		}
		else		// btn released
		{
...
#ifdef CONFIG_FASTPOWER
		pwrbtn_dev = &ipcdev->dev;
        hrtimer_init(&priv->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
        priv->timer.function = fastpower_timer_function;
#endif
