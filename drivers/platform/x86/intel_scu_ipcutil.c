...
#define OSHOB_COLDOFF_REASON_OFFSET     0x34
...
struct scu_ipc_oshob_info {
	...
	__u8    coldoff_reason; /* Pointer to cold off reason log by SCU     */
...
int intel_scu_ipc_read_oshob_info(void)
{
	...
	if (strncmp(oshob_magic,
		...
		poshob_info->coldoff_reason = readl(oshob_addr +
					OSHOB_COLDOFF_REASON_OFFSET);
		pr_info(
			"cold off reason = 0x%x\n",
			poshob_info->coldoff_reason);
...
//AndyPan add
	{
	/* Clear the alarm flag in OSNIB */
	u8 tmp = 0;
       if (poshob_info->oshob_type == OSHOB_STRUCT_OLD)
               ret = intel_scu_ipc_write_osnib(&tmp,
                                               1,
                                               OSNIB_ALARM_OFFSET);
       else
               ret = intel_scu_ipc_write_osnib_extend(&tmp,
                                               1,
                                               OSNIB_ALARM_OFFSET);

       if (ret != 0)
		pr_warn("[BOOT] Clear ALARM FLAG failed\n");
	}
