...
#define DEBUG_I2C_IRQ

#ifdef DEBUG_I2C_IRQ	

#define I2C_MONITOR_IRQ_1 (45)
#define I2C_MONITOR_IRQ_2 (46)

u32 g_debug_irq = 0;
u32 g_isr_count[2]={0,0}; 
u32 g_xfer_count[6]={0,0,0,0,0,0}; // tx_buf_len, buf_len , msgs[].len
...
#ifdef DEBUG_I2C_IRQ	
	if(dev->irq == I2C_MONITOR_IRQ_1)
	{
		g_xfer_count[0] = dev->tx_buf_len;
		g_xfer_count[1] = buf_len;   
		g_xfer_count[2] =  tx_len;
	}
	if( dev->irq == I2C_MONITOR_IRQ_2)
	{
		g_xfer_count[3] = dev->tx_buf_len;
		g_xfer_count[4] = buf_len;    
		g_xfer_count[5] =  tx_len;
	}
#endif 
	/*
...
i2c_dw_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	if (timeout == 0) {
...
#ifdef DEBUG_I2C_IRQ	
	        g_debug_irq = dev->irq;
		dev_warn(dev->dev, "dump IRQ/ISR: sum(irq#%d)=%ld," 
			            "sum(isr%d)=%ld, tx_buf_len=%d, buf_len=%d, msg.len=%d\r\n" 
				   "dump IRQ/ISR: sum(irq#%d)=%ld," 
				   "sum(isr%d)=%ld, tx_buf_len=%d, buf_len=%d, msg.len=%d \n", \
								 I2C_MONITOR_IRQ_1, kstat_irqs(I2C_MONITOR_IRQ_1), \
								 I2C_MONITOR_IRQ_1, g_isr_count[0], \
								 g_xfer_count[0],g_xfer_count[1], g_xfer_count[2], \
								 I2C_MONITOR_IRQ_2, kstat_irqs(I2C_MONITOR_IRQ_2), \
								 I2C_MONITOR_IRQ_2, g_isr_count[1], \	
								 g_xfer_count[3],g_xfer_count[4], g_xfer_count[5] );
		g_xfer_count[0]=0;
		g_xfer_count[1]=0;
		g_xfer_count[2]=0;
		g_xfer_count[3]=0;
		...
		i2c_dw_dump(dev);
#ifdef DEBUG_I2C_IRQ	
	        intel_scu_ipc_send_command(0xCD);
#endif 
		i2c_dw_init(dev);
		ret = -ETIMEDOUT;
		goto done;
...
#ifdef DEBUG_I2C_IRQ	
//enable it to test purpose
#if     0 


	if(dev->irq == I2C_MONITOR_IRQ_1){
	    dev_warn(dev->dev, "count = %d\n", count);
            if(++count == 2000){
	        g_debug_irq = dev->irq;
...
irqreturn_t i2c_dw_isr(int this_irq, void *dev_id)
{
...
#ifdef DEBUG_I2C_IRQ	
	if(this_irq == g_debug_irq){
	    dev_err(dev->dev, "%s:  %s irq=%d enabled= 0x%x stat=0x%x\n", __func__,
		dev->adapter.name, this_irq, enabled, stat);
	...
	if (!enabled || !(stat & ~DW_IC_INTR_ACTIVITY)) {
		pm_runtime_put_autosuspend(dev->dev);
		dev_warn(dev->dev, "%s:  %s IRQ_NONE enabled= 0x%x stat=0x%x\n", __func__,
		dev->adapter.name, enabled, stat);
		return IRQ_NONE;
	}
#ifdef DEBUG_I2C_IRQ	
	if(this_irq == I2C_MONITOR_IRQ_1 )
	    g_isr_count[0]+=1;
	if(this_irq== I2C_MONITOR_IRQ_2 )
	    g_isr_count[1]+=1;
#endif 	
...
tx_aborted:
	if ((stat & (DW_IC_INTR_TX_ABRT | DW_IC_INTR_STOP_DET))
		...
		if(this_irq == g_debug_irq){
		    ...
		    dev_warn(dev->dev, "%s: I2C-irq[%d] recovery OK\n", __func__, g_debug_irq);
		    g_debug_irq = 0;
